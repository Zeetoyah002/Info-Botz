 ...
 case 'nsfwloli':
   reply(mess.wait)
					loli.getNSFWLoli(async (err, res) => {
						...
						Christ.sendMessage(from, buffer, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli om'})
					})
					break
 case 'loli':
          reply(mess.wait)
					loli.getSFWLoli(async (err, res) => {
						if (err) return reply('❌ *ERROR* ❌')
						buffer = await getBuffer(res.url)
						Christ.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ingat! Citai Lolimu'})
					})
					break
 case 'film':
